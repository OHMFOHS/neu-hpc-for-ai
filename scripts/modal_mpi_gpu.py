...
image = (
    modal.Image.from_registry(f"nvidia/cuda:12.8.0-devel-ubuntu22.04", add_python="3.11")
        ...
        .add_local_dir("include", remote_path="/root/include")
...
def compile_and_run_cuda(code_path: str,gpu_count):
    import subprocess
    import threading
    ...
    cmd = ["nvcc", "-DCUDA=1", "-g", "-G", "-rdc=true",
           ...
           "-lstdc++",
           "-I/root/include",
           "-ccbin", "mpicc", "-o", "output.bin", code_path,  "-o", "output.bin",
    "-L/usr/lib/x86_64-linux-gnu", "-lnccl", "-lcudart", "-lm"]

    subprocess.run(cmd, text=True,  check=True)

    print("running program")
    subprocess.run([
    "mpirun", "--allow-run-as-root", "-np", str(gpu_count),
    "./output.bin", str(gpu_count)
